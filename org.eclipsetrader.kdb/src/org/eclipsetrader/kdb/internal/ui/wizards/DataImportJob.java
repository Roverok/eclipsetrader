...
import org.eclipsetrader.kdb.internal.core.connector.BackfillConnector;
                ...
                try {
                    ...
                    if (identifier != null) {
                        ...
                        if (dataMap.size() == timeSpan.length) {
                            ...
                            Status status = new Status(IStatus.WARNING, KdbActivator.PLUGIN_ID, 0, "Missing data for " + security.getName(), null);
                            KdbActivator.log(status);
                        }
                    }
                } catch (Exception e) {
                    Status status = new Status(IStatus.ERROR, KdbActivator.PLUGIN_ID, 0, "Error downloading data for " + security, e);
                    KdbActivator.log(status);
                }
