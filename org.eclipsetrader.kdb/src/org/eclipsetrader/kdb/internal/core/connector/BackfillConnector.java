...
public class BackfillConnector implements IBackfillConnector, IExecutableExtension {
    ...
    private SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd"); //$NON-NLS-1$
    private SimpleDateFormat df2 = new SimpleDateFormat("yyyyMMdd"); //$NON-NLS-1$
    private SimpleDateFormat dfAlt = new SimpleDateFormat("yyyy-MM-dd", Locale.JAPAN); //$NON-NLS-1$
...
//        }
    ...
    void readBackfillStream(List<OHLC> list, BufferedReader in) throws IOException {
        ...
        while ((inputLine = in.readLine()) != null) {
            ...
            try {
                ...
                Status status = new Status(IStatus.ERROR, KdbActivator.PLUGIN_ID, 0, "Error parsing data: " + inputLine, e);
                KdbActivator.log(status);
            }
        }
    }
    ...
    protected OHLC parseResponseLine(String inputLine) throws ParseException {
        ...
        double close = pf.parse(item[4].replace(',', '.')).doubleValue();
        double high = pf.parse(item[2].replace(',', '.')).doubleValue();
        double low = pf.parse(item[3].replace(',', '.')).doubleValue();
        double open = pf.parse(item[1].replace(',', '.')).doubleValue();
    ...
    protected OHLC parse1DayResponseLine(String inputLine) throws ParseException {
        ...
        Date date = new Date(Long.parseLong(item[0] + " " + item[1]) * 1000);
        double close = pf.parse(item[5].replace(',', '.')).doubleValue();
        double high = pf.parse(item[3].replace(',', '.')).doubleValue();
        double low = pf.parse(item[4].replace(',', '.')).doubleValue();
        double open = pf.parse(item[2].replace(',', '.')).doubleValue();
        long volume = nf.parse(item[6]).longValue();
