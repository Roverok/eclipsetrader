...
package org.eclipsetrader.kdb.internal.core.connector;
...
import org.eclipsetrader.kdb.internal.core.Util;
import org.eclipsetrader.kdb.internal.core.repository.IdentifierType;
import org.eclipsetrader.kdb.internal.core.repository.IdentifiersList;
import org.eclipsetrader.kdb.internal.core.repository.PriceDataType;
...
public class SnapshotConnector implements Runnable, IFeedConnector, IExecutableExtension, PropertyChangeListener {
    ...
    public SnapshotConnector() {
        ...
        dateTimeParser = new SimpleDateFormat("yyyy-MM-dd H:mm"); //$NON-NLS-1$
        dateTimeParser.setTimeZone(timeZone);

        dateParser = new SimpleDateFormat("yyyy-MM-dd"); //$NON-NLS-1$
        dateParser.setTimeZone(timeZone);

        timeParser = new SimpleDateFormat("H:mm"); //$NON-NLS-1$
    ...
    public void disconnect() {
        ...
        if (thread != null) {
            try {
                ...
                Status status = new Status(IStatus.ERROR, KdbActivator.PLUGIN_ID, 0, "Error stopping thread", e);
    ...
    protected void fetchLatestSnapshot(HttpClient client, String[] symbols, boolean isStaleUpdate) {
        ...
        try {
            ...
            try {
                ...
                Status status = new Status(IStatus.WARNING, KdbActivator.PLUGIN_ID, 0, "Connection wasn't closed cleanly", e);
                KdbActivator.log(status);
            }
        }
    }
    ...
    protected Date getDateValue(String dateValue, String timeValue) {
        ...
        try {
            ...
            Status status = new Status(IStatus.ERROR, KdbActivator.PLUGIN_ID, 0, "Error parsing date/time values", e);
            KdbActivator.log(status);
        }
    ...
    protected Double getDoubleValue(String value) {
        try {
            ...
            Status status = new Status(IStatus.ERROR, KdbActivator.PLUGIN_ID, 0, "Error parsing number", e);
            KdbActivator.log(status);
        }
        return null;
    }
