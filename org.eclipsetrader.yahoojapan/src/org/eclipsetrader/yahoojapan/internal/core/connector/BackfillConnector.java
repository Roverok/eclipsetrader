...
//                    readDailyBackfillStream(list, in);
    ...
    protected OHLC parseResponseLine(String inputLine) throws ParseException {
        ...
        double close = pf.parse(item[4]).doubleValue();
        double high = pf.parse(item[2]).doubleValue();
        double low = pf.parse(item[3]).doubleValue();
        double open = pf.parse(item[1]).doubleValue();
    ...
    public IDividend[] backfillDividends(IFeedIdentifier identifier, Date from, Date to) {
        ...
        try {
        	for (int page = 1; page < 21; page++) {
                HttpMethod method = Util.getDividendsHistoryMethod(identifier, from, to, page);
                ...
                while ((inputLine = in.readLine()) != null) {
                    if (inputLine.indexOf("<!--/時系列-->") >= 0) {
                        ...
                        for (int i = 1; i < work.length; i++) {
                            try {
                                Dividend dividend = parseDividendsResponseLine(work[i]);
                                if (dividend != null) {
                                    list.add(dividend);
                                }
                            } catch (ParseException e) {
    ...
    protected Dividend parseDividendsResponseLine(String inputLine) throws ParseException {
        ...
        String work = item[1].substring(item[1].indexOf("->") + 3);
        work = work.substring(0, work.indexOf("株") - 1);
        return new Dividend(day.getTime(), pf.parse(work.replace(',', '.')).doubleValue());
    }
