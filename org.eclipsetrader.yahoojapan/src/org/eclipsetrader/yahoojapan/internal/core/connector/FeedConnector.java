...
public class FeedConnector implements IFeedConnector, IExecutableExtension {
    ...
    private IPropertyChangeListener propertyChangeListener = new IPropertyChangeListener() {
        ...
        public void propertyChange(PropertyChangeEvent event) {
            if (YahooJapanActivator.PREFS_DRIVER.equals(event.getProperty())) {
    ...
    public void setInitializationData(IConfigurationElement config, String propertyName, Object data) throws CoreException {
        ...
        String className = YahooJapanActivator.getDefault().getPreferenceStore().getString(YahooJapanActivator.PREFS_DRIVER);
        try {
            connector = (SnapshotConnector) Class.forName(className).newInstance();
        } catch (Exception e) {
            YahooJapanActivator.log(new Status(IStatus.ERROR, YahooJapanActivator.PLUGIN_ID, 0, "Error loding driver " + className, e));
            connector = new SnapshotConnector();
        }
        YahooJapanActivator.getDefault().getPreferenceStore().addPropertyChangeListener(propertyChangeListener);
    }
